...
def test_create_task(service, task_repo):
    task = Task(user_id=1, name="task", type="set_time", start_time="10:00", duration_minutes=30)
    service.create_task(task)
    assert task_repo.tasks[0].name == "task"

def test_get_tasks_for_user(service, task_repo):
    task1 = Task(user_id=1, name="task1", type="set_time", start_time="10:00", duration_minutes=30)
    task2 = Task(user_id=2, name="task2", type="open_time", start_time=None, duration_minutes=20)
...
def test_delete_all_tasks_for_user(service, task_repo):
    t1 = Task(user_id=1, name="t1", type="set_time", start_time="10:00", duration_minutes=30)
    t2 = Task(user_id=1, name="t2", type="open_time", start_time=None, duration_minutes=20)
    t3 = Task(user_id=2, name="t3", type="passive", start_time=None, duration_minutes=10)
    task_repo.create(t1)
    task_repo.create(t2)
    task_repo.create(t3)
